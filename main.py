...
#   # Default to a single setup if not provided
#
...
#     results = []
#     solution_time_start = int(time() * 1000)
#     last_node_visited = None  # Track the last visited node
#
...
#         B, set_1, set_2 = create_directed_bipartite_graph(a_to_b_matrix)
#
#         # Adjust start node for continued execution
#         if last_node_visited and i > 0:
#             print(f"Continuing from last visited node: {last_node_visited}")
#             end_node = last_node_visited  # Start from where the last phase ended
#
#         print(set_1, set_2)
#         method = data["method"]
#         phase_results = {}
...
#             simple_tour = nearest_tsp(B, start_node, end_node, set_1, set_2)
#             simple_tour_cost, time_details = total_cost(B, simple_tour)
#             phase_results["nearest"] = {"tour": simple_tour, "cost": simple_tour_cost, "time_details": time_details}
#             last_node_visited = simple_tour[-1]  # Update last visited node
#
#         if method == "2-opt" or method == "all":
#             print("Running 2-opt TSP...")
#             optimized_tour, optimized_tour_cost = two_opt_for_bipartite(simple_tour, B, set_1, set_2)
#             phase_results["2-opt"] = {"tour": optimized_tour, "cost": optimized_tour_cost}
#             last_node_visited = optimized_tour[-1]  # Update last visited node
#
#         if method == "exact" or method == "all":
#             print("Running Exact Method TSP...")
#             exact_input = {"data": {"distanceMatrix": filtered_matrix, "start_node": start_node, "end_node": end_node}}
#             exact_tour, exact_tour_cost, time_details = run_exact_tsp(exact_input)
#             phase_results["exact"] = {"tour": exact_tour, "cost": exact_tour_cost, "time_details": time_details}
#             last_node_visited = exact_tour[-1]  # Update last visited node
#
...
#         if last_node_visited and i > 0:
#             print(f"Continuing from last visited node: {last_node_visited}")
#             start_node = last_node_visited  # Start from the last node instead of 0.0
#
#         method = data["method"]
#         phase_results = {}
#
...
#             exact_tour, exact_tour_cost, time_details = run_exact_tsp(exact_input)
#
#             # If this is **not** the last phase, remove last movements to `0.0.0 → 0.0`
#             if i < len(kh_sequences) - 1:
#                 print("Removing last movement to 0.0.0 and 0.0 for phase transition.")
#                 for j in range(len(exact_tour) - 1, -1, -1):
#                     if exact_tour[j][1] == "0.0.0":
#                         last_node_visited = exact_tour[j][0]  # Store the last real node
#                         exact_tour = exact_tour[:j]  # Remove unnecessary moves
#                         break
#
#             phase_results["exact"] = {"tour": exact_tour, "cost": exact_tour_cost, "time_details": time_details}
