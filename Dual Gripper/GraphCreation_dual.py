...
def create_directed_bipartite_graph(a_to_b_matrix, large_number=1000000):
    """
    ...
    # # KH → KH edges (excluding any edge involving 0.0)
    # skipped = 0
    # for kh1 in set_1:
    #     for kh2 in set_1:
    #         if kh1 != kh2 and kh1 != "0.0" and kh2 != "0.0":
    #             try:
    #                 distance = a_to_b_matrix.loc[kh1, kh2]
    #             except KeyError:
    #                 try:
    #                     distance = a_to_b_matrix.loc[kh2, kh1]
    #                 except KeyError:
    #                     distance = None
    #             if distance is None or distance >= 1000000:
    #                 print(f"⛔ Skipped due to high weight or missing: {kh1} → {kh2} | weight = {distance}")
    #                 skipped += 1
    #             else:
    #                 B.add_edge(kh1, kh2, weight=distance)
    #
    # print(f"🔍 Total skipped or missing KH→KH edges: {skipped}")
...
for i, kh_setup in enumerate(kh_sequences):
    ...
    B, set_1, set_2 = create_directed_bipartite_graph(matrix)
...
def print_sample_edges_with_weights(B, sample_size=4):
    categories = {
        "KH → KH": [],
        "GR → GR": []
    }
